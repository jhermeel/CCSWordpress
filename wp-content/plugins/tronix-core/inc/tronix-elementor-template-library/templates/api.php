...
class Api extends Source_Base {
    ...
    const LIBRARY_OPTION_KEY = 'tronix_library_info';

    const LIBRARY_TIMESTAMP_CACHE_KEY = 'tronix_remote_update_timestamp';

    //Change Template link
    const API_INFO_URL = 'https://data.themepul.dev/tronix/';

    public function get_id() {
        return 'tronix-library';
    }

    public function get_title() {
        return esc_html__( 'Tronix Library', 'tronixcore' );
    }
    ...
    public function get_data( array $args, $context = 'display' ) {
        ...
        if ( empty( $data ) || empty( $data['content'] ) ) {
            throw new \Exception( __( 'Template does not have any content', 'tronixcore' ) );
        }
